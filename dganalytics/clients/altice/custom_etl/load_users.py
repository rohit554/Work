...
if __name__ == '__main__':
    ...
    if input_file.endswith(".xlsx"):
        users = pd.read_excel(os.path.join(tenant_path, "data", "raw", "user_management", input_file), skiprows=6, engine='openpyxl', usecols=lambda x: 'Unnamed: 0' not in x)
        
    elif input_file.endswith(".csv"):
        users = pd.read_csv(os.path.join(tenant_path, "data", "raw", "user_management", input_file), skiprows=6, usecols=lambda x: 'Unnamed: 0' not in x)
    ...
    users.rename(columns={'middle name': 'middle_name', 'license id': 'license_id', 'Full Name': 'Full_Name', 'Communication Email': 'Communication_Email', 'campaign': 'LOB', 'Team Leader User Id': 'Team_Leader_User_Id', 'Team Manager User Id': 'Team_Manager_User_Id'}, inplace=True)

    users.fillna({'password': '', 'license_id': '', 'middle_name': '', 'contact_info.address': '', 'contact_info.city': '', 'contact_info.country': ''}, inplace=True)
    ...
    createUsersDF = spark.sql(f"""
        SELECT  '' AS password,
            ...
            CASE WHEN U.gender = 'NA' THEN '' ELSE U.gender END gender,
            U.user_id,
            date_format(to_date(U.user_start_date,'dd-MM-yyyy'),'dd-MM-yyyy') AS user_start_date,
            U.email,
            '' `contact_info.address`,
            '' `contact_info.city`,
            '' `contact_info.country`,
