...
if __name__ == '__main__':
    ...
    column_renaming = {
        ...
        "XSR %": "XSR",
        "AICR Queue": "AICR",
        "Ave. Hold (Comcast)": "Hold",
        "ACW (Comcast)": "ACW",
        "AHT (Comcast)": "AHT",
        "Ave. Talk (Comcast)": "Talk",
        "Show Rate": "Showrate",
        "Transfer % (Comcast)": "Xfer",
        "Short Calls %": "SC%",
        "CpC": "CPC",
        "OB % (Comcast)": "OB",
        "Sum of DG Score": "SumofDGScore",
        "Sum of DG Count": "SumofDGCount",
        "Sum of PKT Score": "SumofPKTScore",
        "Sum of PKT Count": "SumofPKTCount",
        "Sum of IR Score": "SumofIRScore",
        "Sum of IR Count": "SumofIRCount"
    }
    ...
    columns = [
              "Name","Emp_ID","Sentiment","Composite","NTT","XSR","AICR","tNPS","CPR","Hold","ACW","AHT","Talk","Showrate","Xfer","SC%","CPC", "OB","Calls","XM_Lines","SumofDGScore","SumofDGCount","SumofPKTScore","SumofPKTCount","SumofIRScore","SumofIRCount"
          ]
    ...
    double_columns = ["Sentiment", "Composite", "NTT", "XSR", "CPR", "Showrate", "Xfer", "SC%", "OB", "AICR","Calls","XM_Lines","SumofDGScore","SumofDGCount","SumofPKTScore","SumofPKTCount","SumofIRScore","SumofIRCount"]
    for column in percentage_columns:
        kpi_spark = kpi_spark.withColumn(column, col(column).cast("double"))
    kpi_spark.createOrReplaceTempView("mtd_metrics")
    mtd_metrics_data = spark.sql(f"""
                                select
                                    ...
                                    coalesce(CPR, "") as CPR,
                                    coalesce(Calls, "") as `Calls`,
                                    coalesce(XM_Lines, "") as `XM_Lines`,
                                    coalesce(SumofDGScore, "") as `SumofDGScore`,
                                    coalesce(SumofDGCount, "") as `SumofDGCount`,
                                    coalesce(SumofPKTScore, "") as `SumofPKTScore`,
                                    coalesce(SumofPKTCount, "") as `SumofPKTCount`,
                                    coalesce(SumofIRScore, "") as `SumofIRScore`,
                                    coalesce(SumofIRCount, "") as `SumofIRCount`
                                from mtd_metrics
                            """)
