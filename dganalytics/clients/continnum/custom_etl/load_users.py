...
if __name__ == '__main__':
    ...
    customer = 'continnum'
    spark = get_spark_session('user_management', customer)
    ...
    if input_file.endswith(".xlsx"):
        ...
        users = pd.read_csv(os.path.join(tenant_path, "data", "raw", "user_management", input_file), skiprows=6, usecols=lambda x: 'Unnamed: 0' not in x, encoding='ISO-8859-1', error_bad_lines=False)
    ...
    users.fillna({'password': '', 'license_id': '', 'middle_name': '', 'contact_info.address': '', 'contact_info.city': '', 'contact_info.country': '', 'dateofbirth' : ''}, inplace=True)

    users.insert(19, 'orgId', 'continnum')
    ...
    AND (U.LOB IN ('Windstream') OR U.LOB IS NULL)
    AND orgId = 'continnum'
    """)
    ...
    createUsersDF = spark.sql(f"""
        SELECT  '' AS password,
            ...
            CASE WHEN U.gender = 'NA' THEN '' ELSE U.gender END gender,
            U.user_id,
            date_format(to_date(U.user_start_date, 'd-MM-yyyy'), 'dd-MM-yyyy') AS user_start_date,
            U.email,
            '' `contact_info.address`,
            '' `contact_info.city`,
            '' `contact_info.country`,
