...
from dganalytics.clients.digicall.export_data import digicall_export_sql
...
if __name__ == "__main__":
    ...
    try:
        ...
        if export_name.startswith("fact_"):
            df = spark_session.sql(digicall_export_sql.__dict__[export_name](extract_start_time, extract_end_time))
        else:
            df = spark_session.sql(digicall_export_sql.__dict__[export_name]).coalesce(1)
        ...
        sftp_username = get_secret(f"{tenant}SFTPUsername")
        sftp_password = get_secret(f"{tenant}SFTPPassword")
