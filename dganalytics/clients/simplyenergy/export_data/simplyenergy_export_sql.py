...
dim_routing_queues = """
    SELECT 
        ...
        cast(mediaSettings.call.serviceLevel.durationMs AS float) AS callServiceLevelDurationMs,
        cast(mediaSettings.call.serviceLevel.percentage * 100 AS float) AS callServiceLevelPercentage,
        cast(mediaSettings.callback.serviceLevel.durationMs AS float) AS callbackServiceLevelDurationMs,
        cast(mediaSettings.callback.serviceLevel.percentage * 100 AS float) AS callbackServiceLevelPercentage,
        cast(mediaSettings.chat.serviceLevel.durationMs AS float) AS chatServiceLevelDurationMs,
        cast(mediaSettings.chat.serviceLevel.percentage * 100 AS float) AS chatServiceLevelPercentage,
        cast(mediaSettings.email.serviceLevel.durationMs AS float) AS emailServiceLevelDurationMs,
        cast(mediaSettings.email.serviceLevel.percentage * 100 AS float) AS emailServiceLevelPercentage,
        cast(mediaSettings.message.serviceLevel.durationMs AS float) AS messageServiceLevelDurationMs,
...
def fact_conversation_metrics(extract_start_time: str, extract_end_time: str):
    return f"""
        SELECT 
            ...
            messageType,
            direction,
            element_at(sessionSegments, 1).queueId AS queueId,
        ...
        GROUP BY
            ...
            messageType,
            direction,
            element_at(sessionSegments, 1).queueId,
