...
def get_conversations(spark: SparkSession, extract_Date: str, extract_start_time: str, extract_end_time: str) :
    conversations_df = spark.sql(f"""
       SELECT   DISTINCT  C.conversationId,
        ...
        WHERE RS.extractDate = '{extract_Date}' 
...
def exec_speechandtextanalytics_transcript(spark: SparkSession, tenant: str, extract_Date: str, extract_start_time: str, extract_end_time: str):
    global logger
    ...
    logger.info(f"Getting conversationId and communicationId extracted for {extract_Date} for Speech And Text Analytics Transript")
    conversations = get_conversations(spark, extract_Date, extract_start_time, extract_end_time)
