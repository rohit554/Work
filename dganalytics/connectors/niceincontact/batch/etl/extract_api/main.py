...
from dganalytics.connectors.niceincontact.batch.etl.extract_api.media_playback_contact import fetch_media_playback_contact
from dganalytics.connectors.niceincontact.batch.etl.extract_api.segments_analyzed_transcript import fetch_segments_analyzed_transcript
...
if __name__ == "__main__":
    ...
    try:
        ...
        if api_name in ["agents", "teams", "teams_agents", "teams_performance",
            ...
            fetch_media_segments(spark, tenant, api_name, run_id, extract_start_time, extract_end_time)
        elif api_name == "media_playback_contact":
            fetch_media_playback_contact(spark, tenant, api_name, run_id,
                             extract_start_time, extract_end_time)
        # elif api_name == "contacts_contactId_email_transcript":
        #     get_master_contact_id_for_email_transcript(spark, tenant, api_name, run_id, extract_start_time, extract_end_time)
        elif api_name == "segments_analyzed_transcript":
            fetch_segments_analyzed_transcript(spark, tenant, api_name, run_id, extract_start_time, extract_end_time)
        elif api_name in ["data_extraction_qm_workflows", "data_extraction_wfm_payroll"]:
            data_extract_jobs(spark, tenant, api_name, run_id, extract_start_time, extract_end_time)
        else:
