...
def exec_case(
    ...
    logger = sf_utils_logger(tenant, "case_job")
    logger.info("case job inside")
    api_headers = authorize(tenant)
    body = {
        "operation": "query",
        "query": f"SELECT Id,AccountId, Comments, CaseNumber, CreatedDate, ClosedDate, ContactEmail, ContactFax, ContactId, ContactMobile, ContactPhone, Description, IsClosed, IsEscalated, Origin, OwnerId, Priority, Reason, Status, Subject, Type, AssetId, SourceId, CreatedById, EngineeringReqNumber__c, LastModifiedById, Linked_Order__c, ParentId, PotentialLiability__c, Product__c, SLAViolation__c, SuppliedCompany, SuppliedEmail, SuppliedName, SuppliedPhone FROM CASE where LastModifiedDate >= {extract_start_time} AND LastModifiedDate <= {extract_end_time}",
        "contentType": "CSV",
    }
    ...
    if job_resp.status_code != 200:
        print("case Details Job Submit API Failed", job_resp.text)
        return
    ...
    if pandas_df.empty:  # Check if the Pandas DataFrame is empty
        ...
        df = (
            df.withColumn("IsClosed", col("IsClosed").cast("boolean"))
            .withColumn("IsEscalated", col("IsEscalated").cast("boolean"))
            .withColumn("SLAViolation__c", col("SLAViolation__c").cast("boolean"))
        )
