...
def exec_task(
    ...
    logger = sf_utils_logger(tenant, "task_job")
    logger.info("task job inside")
    api_headers = authorize(tenant)
    body = {
        "operation": "query",
        "query": f"SELECT OwnerId, CallDurationInSeconds, CallObject, CallDisposition, CallType, Description, CompletedDateTime, IsRecurrence, CreatedById, ActivityDate, LastModifiedById, WhoId, Priority, RecurrenceInterval, WhatId, IsReminderSet, RecurrenceRegeneratedType, Status, Subject, TaskSubtype from Task where LastModifiedDate >= {extract_start_time} AND LastModifiedDate <= {extract_end_time}",
        "contentType": "CSV",
    }
