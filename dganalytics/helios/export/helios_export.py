...
def helios_export(spark, tenant, extract_name, output_file_name):
    ...
    try:
        ...
        SELECT a.conversationId,
            ...
            a.eventStart,
            a.eventEnd,
            case when f.exitReason != 'TRANSFER' then 'Ended in IVR' else a.contact_reason end as contact_reason,
            case when f.exitReason != 'TRANSFER' then 'Ended in IVR' else a.main_inquiry end as main_inquiry,
            case when f.exitReason != 'TRANSFER' then 'Ended in IVR' else a.root_cause end as root_cause,
            a.location,
        ...
        LEFT JOIN (SELECT case when exitReason = 'FLOW_DISCONNECT' THEN 'resolved' END as resolved, 
                          conversationId,
                          exitReason from dgdm_{tenant}.dim_conversation_session_flow
                  ) f
