...
def hf_value_stream_export(spark, tenant, extract_name, output_file_name):
    logger = helios_utils_logger(tenant, "coversation map")
    ...
    try:
        ...
        df = spark.sql(
            f"""
                ...
                coalesce(startTime,endTime) startTime,
                coalesce(endTime,startTime) endTime
                from dgdm_{tenant}.fact_value_stream
        """
        )
        # df.display()
        df = df.toPandas()
