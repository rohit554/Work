from dganalytics.utils.utils import (
    exec_mongo_pipeline,
...
pipeline = [
    {"$match": {"org_id": {"$in": get_active_org()}}},
    {
        "$project": {
            "user_id": 1.0,
            "email": 1.0,
    ...
    {"$unwind": {"path": "$works_for", "preserveNullAndEmptyArrays": True}},
    {
        "$lookup": {
            "from": "Organization",
    ...
    {"$unwind": {"path": "$Team", "preserveNullAndEmptyArrays": True}},
    {
        "$lookup": {
            ...
            "pipeline": [
                ...
                {"$project": {"name": 1.0, "user_id": 1.0, "works_for": 1.0}},
                {"$unwind": {"path": "$works_for"}},
                {
                    "$match": {
                        "$expr": {
                            "$and": [
                                {"$eq": ["$works_for.role_id", "Team Lead"]},
                                {"$eq": ["$works_for.team_id", "$works_for.team_id"]},
                            ]
                        }
                    }
                },
    ...
    {"$unwind": {"path": "$tl_data", "preserveNullAndEmptyArrays": True}},
    {
        "$project": {
            "_id": 0.0,
            "MongoUserId": "$_id",
            ...
            "Name": "$name",
            "Quartile": "$quartile",
            "RoleId": {"$ifNull": ["$works_for.role_id", "$role_id"]},
            "TeamName": "$Team.name",
...
schema = StructType(
    [
        ...
        StructField(
            "MongoUserId", StructType([StructField("oid", StringType(), True)]), True
        ),
...
def get_users(spark):
    ...
    """
    delta_table_partition_ovrewrite(df, "dg_performance_management.users", ["orgId"])